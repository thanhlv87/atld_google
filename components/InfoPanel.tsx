  ...
  return (
    <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-md h-full">
      <div className="flex items-center mb-4">
        <div
          className={`flex-shrink-0 h-10 w-10 flex items-center justify-center ${currentTheme.iconBg} rounded-full mr-3`}
        >
        ...
        <ul className="space-y-3">
          {items.map((item, index) => (
            <li key={index} className="flex justify-between items-center text-sm">
              ...
              {item.details && (
                <span className="font-bold text-neutral-dark bg-gray-100 px-2 py-0.5 rounded-md">
                  {item.details}
                </span>
              )}
            </li>
